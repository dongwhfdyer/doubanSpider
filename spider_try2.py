...
def crawl_data(url):
    ...
    url = 'https://movie.douban.com/subject/1433330/comments'+url
...
def parse(response):
    ...
    review_list = soup.find_all('div', {'class': 'comment-item '})

    for review_div in review_list:
        # 作者
        author = review_div.find('a')['title']
        # 发布时间
        pub_time = review_div.find('span', {'class': 'comment-time '}).text.strip()
        # 评分
        rating = review_div.find('span', {'class': 'allstar40 rating'})
        if rating:
        ...
        if is_unfold:
            ...
            content = review_div.find('p', {'class': ' comment-content'}).text
        if content:
